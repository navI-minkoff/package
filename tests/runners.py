...
from utils.tests_utils import parse_standard, parse_spreads, parse_with_individual_lists, parse_with_individual_lists_and_groups, parse_groups
...
def run_case_groups(ps, doc, jpeg_options, parsed, output_dir, album_type):
    prefix = 1
    for group in parsed.get("groups_jpeg", []):
        lists_is_even = len(group["group_jpeg_filenames"]) % 2 == 0
        packagingGroup(
            ps, doc, jpeg_options,
            group["groups_jpeg"],
            group["group_jpeg_filenames"],
            output_dir,
            prefix,
            group["postfix"],
            album_type,
